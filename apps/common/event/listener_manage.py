...
class ListenerManagement:
    ...
    def embedding_by_paragraph_data_list(data_list, paragraph_id_list, embedding_model: Embeddings):
        ...
        # Publish chunk-level progress for the first paragraph's document
        if paragraph_id_list:
            try:
                first_para = QuerySet(Paragraph).filter(id=paragraph_id_list[0]).values("document_id").first()
                if first_para:
                    doc_id = str(first_para["document_id"])
                    IndexProgressPublisher.publish_progress(
                        document_id=doc_id, stage="chunk", status="started", percentage=0,
        ...
        try:
            ...
            # Publish chunk completed
            if paragraph_id_list:
                try:
                    doc_id = str(
                        QuerySet(Paragraph).filter(id=paragraph_id_list[0]).values_list("document_id", flat=True).first()
                    )
                    IndexProgressPublisher.publish_progress(
                        document_id=doc_id, stage="chunk", status="completed", percentage=100,
                        message=f"Chunking completed ({len(paragraph_id_list)} paragraphs)"
                    )
                except Exception:
                    pass
        except Exception as e:
