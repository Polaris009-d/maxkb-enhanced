...
class HybridSearch(ISearch):
    """BM25 + Dense vector hybrid retrieval with RRF (Reciprocal Rank Fusion).

    Runs both embedding similarity search and keyword (BM25-like) search
    independently, then fuses the result rankings using the RRF algorithm
    with k=60. This improves recall for queries with precise keywords.
    """
    ...
    def handle(
        ...
        # 1) Run dense embedding search
        emb_handler = EmbeddingSearch()
        emb_results = emb_handler.handle(
            query_set, query_text, query_embedding, top_number, similarity, search_mode, knowledge_id_list
        )

        # 2) Run keyword (BM25-like) search
        kw_handler = KeywordsSearch()
        kw_results = kw_handler.handle(
            query_set, query_text, query_embedding, top_number, similarity, search_mode, knowledge_id_list
        )

        # 3) Build rank dictionaries (rank starts at 1)
        emb_ranks = {}
        for idx, r in enumerate(emb_results):
            emb_ranks[r["paragraph_id"]] = idx + 1

        kw_ranks = {}
        for idx, r in enumerate(kw_results):
            kw_ranks[r["paragraph_id"]] = idx + 1

        # 4) RRF fusion: score = 1/(k+rank_dense) + 1/(k+rank_sparse)
        all_ids = set(emb_ranks.keys()) | set(kw_ranks.keys())
        fused = []
        for pid in all_ids:
            rrf_score = 0.0
            if pid in emb_ranks:
                rrf_score += 1.0 / (self.RRF_K + emb_ranks[pid])
            if pid in kw_ranks:
                rrf_score += 1.0 / (self.RRF_K + kw_ranks[pid])
            # Keep similarity from whichever result has it (prefer embedding)
            source = next((r for r in emb_results if r["paragraph_id"] == pid), None) or next(
                (r for r in kw_results if r["paragraph_id"] == pid), None
            )
            fused.append(
                {
                    "paragraph_id": pid,
                    "comprehensive_score": rrf_score,
                    "similarity": source["similarity"] if source else rrf_score,
                }
            )

        fused.sort(key=lambda x: x["comprehensive_score"], reverse=True)
        return fused[:top_number]

    def support(self, search_mode: SearchMode):
        return search_mode.value == SearchMode.hybrid.value


search_handle_list = [EmbeddingSearch(), KeywordsSearch(), BlendSearch(), HybridSearch()]
